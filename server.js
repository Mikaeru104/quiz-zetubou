    ...
    ws.on('message', message=>{
        ...
        if(msg.type==='start'){
            ...
            if(msg.stage===2){
                ...
                else ws.send(JSON.stringify({type:'waiting', message:`第二ステージ: あと ${requiredPlayersStage2-readyCount} 人待ち...`}));
            }

            if(msg.stage===3){
                if(!player.clearedStage2){ ws.send(JSON.stringify({type:'waiting', message:"第二ステージ未クリアのため参加不可"})); return; }
                startStage3([player]);
            }
    ...
    function sendNextQuestion(){
        if(questionIndex<questions.length){
            ...
            stagePlayers.forEach(p=>{
                p.handleAnswer=(plr, answer)=>{
                    if(!plr || plr.answered) return;
                    if(answer.trim()===q.correctAnswer){ plr.scoreStage3+=30; plr.ws.send(JSON.stringify({type:'score', score:plr.scoreStage3})); plr.ws.send(JSON.stringify({type:'waiting', message:'正解'})); }
                    else{ plr.ws.send(JSON.stringify({type:'waiting', message:'不正解'})); }
                    plr.answered=true;
                    if(stagePlayers.every(pl=>pl.answered)){ clearInterval(qTimer); questionIndex++; setTimeout(sendNextQuestion,1000); }
                };
            });
        } else endStage3(stagePlayers);
    }
...
function startStage4(stagePlayers){
    stagePlayers.forEach(p=>{ p.scoreStage4=0; p.answered=false; p.ready=false; p.ws.send(JSON.stringify({type:'showClearButton'})); });
